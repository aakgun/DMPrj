...
def read_data2():
    ...
    df_val = test
    #categories = list(set(df_train.categories.values))
    categories = list(set(dfdrugbank2.category.values))
...
dfdrugbank=pd.read_csv("./data/drugbank.tsv",delimiter="\t")
dfprotein=pd.read_csv("./data/proteins.tsv",delimiter="\t")
...
analyze_predictions(categories,df_train['category'].values,train_predictions)

# Make and analyze validation predictions
val_predictions = one_hot_to_category(model.predict(x_val),categories)
analyze_predictions(categories,df_val['category'].values,val_predictions)
