...
class doc2VecModel(Model):
    ...
    def Tokenization(data, concept=True, stem=True, removeStopwords=True):
        if concept == False:
    ...
    def label_sentences2(corpus, label_type):
        ...
        for i, v in enumerate(corpus):
            label = label_type + '_' + str(i)
            #labeled.append(doc2vec.LabeledSentence(v.split(), [label]))
            labeled.append(doc2vec.LabeledSentence(words=Tokenization(corpus, concept=False, stem=True, removeStopwords=True), tags=[label]))
